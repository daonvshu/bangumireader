## BangumiReader
### 介绍
这是一个[蜜柑](https://mikanani.me/)Rss订阅软件，在这里你可以下载收藏蜜柑网站上的字幕组发布的种子文件，或者查看历年番剧数据库。

### Screenshot
![](screenshot/mikan_bangumi_list_home.png)
![](screenshot/mikan_bangumi_download.png)
![](screenshot/rss_subscribe_page.png)
![](screenshot/bangumi_database_page.png)
![](screenshot/bangumi_database_detail.png)

### 数据源支持
- [蜜柑](https://mikanani.me/)
- [Bangumi Data](https://github.com/bangumi-data/bangumi-data)
- [番组计划](https://bangumi.tv/)

### 第三方插件
- [qtdao](https://github.com/daonvshu/QtDao)
- [qtnetworkservice](https://github.com/aeagean/QtNetworkService)
- [google breakpad](https://chromium.googlesource.com/breakpad/breakpad)

### 编译/运行
- Visual Studio 2022
- Qt5.15.2/qml